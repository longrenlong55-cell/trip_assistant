# 使用AI大模型

import os

from langchain_tavily import TavilySearch
from langchain_openai import ChatOpenAI

# llm = ChatOpenAI(
#     temperature=0,
#     model="GLM-4-0520",
#     openai_api_key="4afc2ced3f174bc89dd17b3e47d2586d.qqcyAW2zEEqj5rY3",
#     openai_api_base="https://open.bigmodel.cn/api/paas/v4/",
# )


# llm = ChatOpenAI(   # 用的自己的服务器部署的大模型
#     temperature=0.8,
#     model="Qwen-7B",
#     openai_api_key="EMPTY",#随意写
#     openai_api_base="http://localhost:6006/v1"
# )

llm = ChatOpenAI(  # openai的
    temperature=0,
    model='gpt-4o',
    api_key="sk-zoPqidhLdgmOqL8F29xNyDzTrKDcDH0mWJcJvd4cgFID1cc1",
    base_url="https://xiaoai.plus/v1")

# llm = ChatOpenAI(  # openai的
#     temperature=0,
#     model='claude-3-7-sonnet-20250219',
#     api_key="sk-doD81WgxSoF9A6xYzhgW7GUh5frRwPETI8mDq3ce4UaWnCPF",
#     base_url="https://xiaoai.plus/v1")

# llm = ChatOpenAI(
#     temperature=0,
#     model='deepseek-chat',
#     api_key="sk-6c90758171ea4cf799171ec689a26444",
#     base_url="https://api.deepseek.com")


# 初始化搜索工具，限制结果数量为2
os.environ["TAVILY_API_KEY"] = "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"
tavily_tool = TavilySearch(max_results=1)